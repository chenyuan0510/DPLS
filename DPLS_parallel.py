...
def dpls(data,y=None,n_job=1):
    if y is not None:
        res = data.swifter.apply(dpls_score, axis=0, y=y)
    else:
        data = pd.DataFrame(data)
        out = Parallel(n_jobs=n_job)(
            delayed(data.swifter.apply)(dpls_score, axis=0, y=data.iloc[:, i])
